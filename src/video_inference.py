...
class VideoFluidSegmentation:
    ...
    def create_visualization(
        ...
        """
        # Resize mask and confidence to match original frame size
        h, w = original_frame.shape[:2]
        mask_resized = cv2.resize(predicted_mask.astype(np.uint8), (w, h))
        conf_resized = cv2.resize(confidence_map, (w, h))
        
        # Create overlay with red mask
        overlay = original_frame.copy()
        red_mask = np.zeros_like(original_frame)
        ...
        # Top: Original + Overlay
        # Bottom: Mask + Confidence
        mask_3ch = cv2.cvtColor((mask_resized * 255).astype(np.uint8), cv2.COLOR_GRAY2BGR)
        
        top_row = np.hstack([original_frame, overlay])
        ...
        if top_row.shape[1] != bottom_row.shape[1]:
            ...
            top_row = cv2.resize(top_row, (min_width, top_row.shape[0] * min_width // top_row.shape[1]))
            bottom_row = cv2.resize(bottom_row, (min_width, bottom_row.shape[0] * min_width // bottom_row.shape[1]))
        ...
        # Labels for top row
        cv2.putText(visualization, 'Original', (10, 30), font, font_scale, color, thickness)
        cv2.putText(visualization, 'Overlay', (w + 10, 30), font, font_scale, color, thickness)
        
        # Labels for bottom row
        cv2.putText(visualization, 'Mask', (10, h + 30), font, font_scale, color, thickness)
        cv2.putText(visualization, 'Confidence', (w + 10, h + 30), font, font_scale, color, thickness)
    ...
    def process_video_realtime(
        ...
        if save_output and output_path:
            ...
            # Output will be twice the height (visualization format)
        ...
        try:
            while True:
                ...
                cv2.putText(visualization, info_text, (10, visualization.shape[0] - 20), 
